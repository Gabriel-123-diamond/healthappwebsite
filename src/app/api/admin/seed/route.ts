...
export async function POST(req: NextRequest) {
  try {
    ...
    // Support both direct password and session token
    ...
    if (adminSession) {
      try {
        ...
        console.warn("Invalid session format in seed route");
      }
    }

    // Check for password in body if not already authenticated
    if (!isAuthenticated) {
      try {
    ...
    const herbalSpecialties = ['Herbal Practitioner', 'Traditional Chinese Medicine Practitioner', 'Naturopath', 'Holistic Therapist', 'Homeopath', 'Acupuncturist', 'Ayurvedic Practitioner'];
    ...
    const types: ('doctor' | 'herbal_practitioner' | 'hospital')[] = ['doctor', 'herbal_practitioner', 'hospital'];
    ...
    for (const type of types) {
      for (let i = 0; i < 50; i++) {
        ...
        } else if (type === 'herbal_practitioner') {
    ...
    return NextResponse.json({ message: `Successfully seeded ${count} global experts/institutions and ${articles.length} articles.` });

  } catch (error: any) {
    console.error("Seeding Error:", error);
    return NextResponse.json({ error: "Failed to seed database due to an internal error." }, { status: 500 });
  }
}
