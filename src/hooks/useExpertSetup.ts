...
import { expertSetupUtils } from './expert-setup/expertSetupUtils';
...
export const useExpertSetup = () => {
    ...
    try {
      ...
      if (user) {
        ...
        const expertUpdate: any = {
          ...
          phones: formData.phones.map(p => ({ ...p, number: p.number.replace(/\s/g, p.number) })),
          ...
          verificationLevel: formData.license.licenseNumber ? 2 : 1,
          tier: formData.tier,
  ...
  return {
    ...
    removeItem,
    updateArrayItem,
    isStepComplete: (s: number) => expertSetupUtils.isStepComplete(s, userProfile),
    getNextAvailableStep: (c: number, d?: 'forward' | 'backward') => expertSetupUtils.getNextAvailableStep(c, userProfile, d),
    isUpgrading: userProfile?.verificationStatus === 'verified'
  };
};
