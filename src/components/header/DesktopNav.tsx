    ...
    <nav className="hidden lg:flex items-center gap-1">
      {NAVIGATION_LINKS.map((link) => (
        <Link 
          key={link.href} 
          href={link.href} 
          className="px-4 py-2 text-sm font-bold text-slate-600 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-all rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/20"
        >
      ...
      <div className="relative" ref={platformMenuRef}>
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold transition-all rounded-xl ${
            platformMenuOpen 
        ...
        >
          Tools
          <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${platformMenuOpen ? 'rotate-180' : ''}`} />
        </button>
